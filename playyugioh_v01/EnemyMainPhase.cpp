...
EnemyMainPhase::EnemyMainPhase(QObject *parent) :
    QObject(parent)
...
void EnemyMainPhase::execute(){
    qDebug() << "Executing Main Phase actions...";

    if(handListWidget){
        disconnect(handListWidget, &QListWidget::itemDoubleClicked, this, &EnemyMainPhase::onCardClicked);
        bool connected = connect(handListWidget, &QListWidget::itemDoubleClicked, this, &EnemyMainPhase::onCardClicked);
        qDebug() << "Connect result:" << connected;

    }
}

void EnemyMainPhase::setHandListWidget(QListWidget *widget){
    handListWidget = widget;
}

void EnemyMainPhase::setFieldListWidget(QListWidget *widget, int index){
    if (index >= 0 && index < fieldListWidgets.size()) {
        fieldListWidgets[index] = widget; // インデックスを指定してウィジェットを設定
    }
}

void EnemyMainPhase::onCardClicked(QListWidgetItem *item){
